...
$emailSubject = "Azure Active Directory Signin Acitivities Report $(Get-Date -Format MM/dd/yyyy)"

$emailBody = "<h2>Azure Active Directory Signin Acitivities Report</h2>`n"
...
$i=0
    ...
    foreach ( $event in $myReport.value ) {
        ...
        if ($event.loginStatus -ne "Success" -and $event.location.state -ne "Your location, state/s abbreviation") {  #This if is to exclude successful logins from approved locations from the email report
